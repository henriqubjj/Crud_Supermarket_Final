...
class GerenciadorCRUD:
    ...
    def adicionar_usuario(self, nome, senha, telefone, endereco, is_flamengo=0, watch_one_piece=0, is_de_sousa=0, is_funcionario=0):
        self.cursor.execute("INSERT INTO usuarios (nome, senha, telefone, endereco, is_flamengo, watch_one_piece, is_de_sousa, is_funcionario) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                            (nome, senha, telefone, endereco, is_flamengo, watch_one_piece, is_de_sousa, is_funcionario))
        self.connection.commit()
    ...
    def adicionar_funcionario(self, nome, senha, cargo, salario):
        self.cursor.execute("INSERT INTO funcionarios (nome, senha, cargo, salario) VALUES (?, ?, ?, ?)", 
    ...
    def login(self, id, senha):
        ...
        self.cursor.execute("SELECT id, nome FROM usuarios WHERE id = ? AND senha = ?", (id, senha))
        usuario = self.cursor.fetchone()
        if usuario:
        ...
        self.cursor.execute("SELECT id, nome FROM funcionarios WHERE id = ? AND senha = ?", (id, senha))
        funcionario = self.cursor.fetchone()
        if funcionario:
