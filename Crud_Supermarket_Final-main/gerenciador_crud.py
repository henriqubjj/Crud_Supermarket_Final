...
class GerenciadorCRUD:
    ...
    def autenticar_usuario(self, email, senha):
        ...
        if usuario:
            columns = ['id', 'nome', 'email', 'senha', 'telefone', 'endereco', 'is_flamengo', 'watch_one_piece', 'is_de_sousa', 'is_funcionario']
            usuario_dict = dict(zip(columns, usuario))
            print(f"Usuário {usuario_dict['nome']} autenticado com sucesso.")
            self.usuario_logado = usuario_dict  # Armazena os detalhes do usuário autenticado
            return usuario_dict
        else:
